...
script_path <- if (length(file_arg)) sub("^--file=", "", file_arg[1]) else "scripts/simulation/02_run_main.R"
...
allow_missing <- as_bool_cli_v2(cli$`allow-missing-benchmarks`, FALSE)

if (!file.exists(file.path(root, "results", "logs", "profile_geometry_validation.csv"))) {
  stop("Geometry validation output is missing. Run 00_validate_profile_geometry.R first.")
}
ans <- run_registry_v2(root, config, run_id, jobs = jobs, max_reps = max_reps,
                       allow_missing_benchmarks = allow_missing)
message("Main run written to: ", ans$run_dir)
